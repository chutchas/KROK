...
- **เอกสารที่เกี่ยวข้อง** — แนบคู่มือ / SOP / drawing / รูปตัวอย่าง ได้ทั้ง **ระดับฟอร์ม** (เห็นทุกขั้นตอน) และ **ระดับฟิลด์** (เห็นใต้ชื่อฟิลด์นั้น) รองรับทั้งไฟล์ (PDF / รูป / MP4 ≤ 20 MB) และลิงก์ภายนอก คนหน้างานกดเปิดดูได้ระหว่างกรอกโดยไม่ออกจากฟอร์ม
- **ทะเบียนอุปกรณ์ + อนุมัติรายเครื่อง** — ติ๊ก “เฉพาะเครื่องที่อนุมัติแล้ว” ตอนสร้างฟอร์ม → เครื่องที่ยังไม่ได้รับอนุมัติเปิดกรอกฟอร์มนั้นไม่ได้ ผู้ดูแลอนุมัติ/เพิกถอนที่ **ตั้งค่า → อุปกรณ์** ทุก submission บันทึกว่ามาจากเครื่องไหน บังคับสิทธิ์ที่ระดับ RLS ไม่ใช่แค่ UI
- **ผูกอุปกรณ์กับฟอร์ม (device × form)** — ฟอร์มที่ล็อคเลือกได้ 2 แบบ: *ทุกเครื่องที่อนุมัติแล้ว* (ค่าเริ่มต้น) หรือ *เฉพาะเครื่องที่เลือก* แบบหลังต้องอนุมัติเครื่อง **และ** ผูกกับฟอร์มนั้นด้วยจึงกรอกได้ จัดการได้ 2 ทาง: ตาราง **เครื่อง × ฟอร์ม** ในหน้าอุปกรณ์ (สลับโหมดจากหัวคอลัมน์ได้เลย) หรือเลือกเครื่องจากหน้าแก้ฟอร์มใน Studio · ปลดล็อคฟอร์มเมื่อไหร่ การผูกจะถูกล้างอัตโนมัติ
  > ข้อจำกัด: ตัวตนของเครื่องผูกกับ localStorage ของเบราว์เซอร์ ไม่ใช่ hardware attestation — ล้างข้อมูลเบราว์เซอร์แล้วต้องขออนุมัติใหม่ และฟอร์มสาธารณะ (QR) ล็อคเครื่องไม่ได้

> **สำคัญ:** ต้องรัน migration **`0025_attachments_devices.sql`** แล้วต่อด้วย **`0026_form_devices.sql`** ก่อนใช้งานฟีเจอร์เหล่านี้
> (0025 สร้างตาราง `form_attachments` / `devices`, bucket `attachments` และแก้ policy `submissions_insert` · 0026 เพิ่มตาราง `form_devices` + `forms.device_scope` และอัปเดตฟังก์ชัน `device_ok()`)
