...
import { Lock, Plug, TriangleAlert, Globe } from "lucide-react";
...
export default function AdminAiClient({ current, configured }: { current: AiSettings; configured: boolean }) {
  ...
  async function save() {
    if (!current.has_key && !apiKey.trim()) { setMsg({ t: "กรุณาใส่ API key", err: true }); return; }
  ...
  async function runTest() {
    setBusy(true); setTest({ t: "กำลังทดสอบเรียก LLM..." });
    try {
